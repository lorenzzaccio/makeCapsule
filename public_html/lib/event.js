...
function mouseDown(e){
    ...
    if(m_locked==0){
        ...
        //var id = el.parentNode.getAttributeNS(null,"id");
        //document.getElementById("verb4").value="id="+id;
        //console.info("id="+id+" el="+el+"selecteditem="+selectedItem);
        var parent = el.parentNode;
        //console.info("parentnodeId="+parent.parentNode.getAttributeNS(null,"id"));
        //if(id=="svgbasics")
        //    return;
        if(bOver==1){
            back_drag=0;
            //set the selected object at the of the others
            //el = subgroup sub_gpal
            //el.parentNode = group gpal
            //el.parentNode.parentNode = group supérieur : SVG ou le groupe de mSelectId
            el.parentNode.parentNode.appendChild(el.parentNode); 
            //as a record :
            ...
            */
            ...
            if((selectedIndexOver != selectedIndex)&&(itemFocused==1)&&(selectedIndexOver!=-1) ){
                ...
                if(m_select==0){
                    ...
                    //transx = getTransX(el.parentNode.parentNode);
                    //transy = getTransY(el.parentNode.parentNode);
                }
            ...
            //testParent(selectedItem);
            //if(parent.parentNode.getAttributeNS(null,"id").parentNode==="mSelectId"){
            ...
            if(m_select===0){
                ...
                if(color==="blue"){
                    ...
                    //addItemToClipboard();
                    /*var svg = $('#svgbasics').svg('get');
                    //var x = parseFloat($(selectedItem).attr('x'));
                    //var idname = $(selectedItem).attr('id');
                    
                    var group = document.getElementById("mSelectId");
                    var tmp = $(selectedItem).removeAttr('id').clone().appendTo(group).attr({'x': x+10, 'id': idname});
                
                     //var node = svg.clone(selectedItem);
                     
                     
                     //svg.add(group,node);
                     svg.remove($(selectedItem));*/
                     //selectedItem.parentNode="mSelectId";
                     //console.info(selectedItem.parentNode);
                     var group = document.getElementById("mSelectId");
            ...
            //$(selectedItem.firstChild).animate({svgFill: 'orange'}, 1000);
